...
class Brick
  ...
  def disintegrate : Int32
    parents     = [ self ]
    all_parents = [ ] of Brick
    falling_bricks = 0
    while parents.size > 0
      children = parents.flat_map(&.@children).uniq
      all_parents = (all_parents + parents).uniq
      orphans = children.select { |child| (child.@parents - all_parents).size == 0 }
      falling_bricks += orphans.size
      parents = orphans
    end
    falling_bricks
  end
...
puts well.@bricks.map(&.disintegrate).sum
